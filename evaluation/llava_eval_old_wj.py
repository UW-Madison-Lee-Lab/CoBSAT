...
def summary(
    ...
    output = "image" if prompt == 1 else "text"
    prompt_dict = {
        1: {
            'prompt1': f'What is the object\'s color in this {output}? Answer from the following options: (1)black (2)blue (3)red (4)yellow (5)green (6)purple (7)pink (8)white',
            'prompt2': f'What is the object in this {output}? Answer from the following options: (1)car (2)leaf (3)box (4)building (5)cup (6)bag (7)flower (8)chair',
        },
        3: {
            'prompt1': f'How many objects are there in this {output}? Answer from the following options: (1)one (2)two (3)three (4)four (5)five (6)six',
            'prompt2': f'What is the object in this {output}? Answer from the following options: (1)apple (2)cat (3)chair (4)cup (5)dog (6)lion (7)person (8)shampoo',
        },
        5: {
            'prompt1': f'What is the style of this {output}? Answer from the following options: (1)cartoon (2)oil (3)sketch (4)cubism (5)watercolor (6)origami',
            'prompt2': f'What is the object in this {output}? Answer from the following options: (1)man (2)cat (3)flower (4)apple (5)dog (6)house (7)car (8)chair',
        },
        7: {
            'prompt1': f'What is the object\'s action in this {output}? Answer from the following options: (1)wink (2)run (3)sing (4)fly (5)sit (6)drink (7)sleep (8)eat',
            'prompt2': f'What is the object in this {output}? Answer from the following options: (1)cat (2)dog (3)pig (4)lion (5)bird (6)monkey (7)man (8)woman',
        },
        9: {
            'prompt1': f'What is the background in this {output}? Answer from the following options: (1)beach (2)street (3)park (4)forest (5)office (6)classroom (7)gym (8)library',
            'prompt2': f'What is the object in this {output}? Answer from the following options: (1)car (2)cat (3)chair (4)dog (5)man (6)monkey (7)robot (8)woman',
        },
    }
    ...
    for data_id in data_ids:
        ...
        for mllm in mllms:
            ...
            for shot in shots:
                print(f"| --- shot {shot}")
                score_per_task = 0
                first_score_per_task = 0
                second_score_per_task = 0                
                ...
                score_per_object = 0
                first_score_per_object = 0
                second_score_per_object = 0
                ...
                for filename in tqdm(all_generated_files):
                    if filename.endswith(".jpg") or filename.endswith(".png"):
                        ...
                        out1 = eval_model(prompt1, file_path, tokenizer, llava_model, image_processor, context_len, llava_args)
                        out2 = eval_model(prompt2, file_path, tokenizer, llava_model, image_processor, context_len, llava_args)
                        ...
                        first_score_per_object += first_score
                        second_score_per_object += second_score
                ...
                sample_mean = {
                    ...
                    "first_score": first_score_per_object/number_per_object,
                    "second_score": second_score_per_object/number_per_object,
                ...
                first_score_per_task += first_score_per_object
                second_score_per_task += second_score_per_object
                strict_score_per_task += strict_score_per_object
                number_per_task += number_per_object

                json_file = f"{folder}/detail/[{prompt}][{data_id}]{task_name}_{mllm}_{shot}shot({(score_per_object/number_per_object):2f})({(first_score_per_object/number_per_object):2f})({(second_score_per_object/number_per_object):2f})({(strict_score_per_object/number_per_object):.2f}).json"
                ...
                sample_mean = {
                    ...
                    "first_score": first_score_per_task/number_per_task,
                    "second_score": second_score_per_task/number_per_task,
                    "strict_score": strict_score_per_task/number_per_task,
                    "number": number_per_task
                }
                samples_mean.append(sample_mean)
                json_file = f"{folder}/[{prompt}][{data_id}]{task_name}_{mllm}_{shot}shot_mean({(score_per_task/number_per_task):.2f})({(first_score_per_task/number_per_task):.2f})({(second_score_per_task/number_per_task):.2f})({(strict_score_per_task/number_per_task):.2f}).json"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mllms', type=str, nargs='+', default=['Emu', 'gill', 'SEED'], help='mllms')
    ...
    parser.add_argument('--prompt', type=int, default=1, help='prompt', choices = [1,2])

...
def eval_description(
    ...
    csv_file_path = f"{root_dir}/results/evals/{model}_text/shot_{shot}{misleading_flag}/task_{task_id}_summary.csv"
    ...
    for count in tqdm(range(max_file_count), desc = f"Evaluating {model}_text/shot_{shot}{misleading_flag}/task_{task_id}"):
            
        input_dict = data_loader[count]
        input_dict['x'] = input_dict['x_list'][-1]
        save_path = f"{folder_path}/{input_dict['save_path']}.json"
        output_dict = read_json(save_path)
